...
db.any('SELECT movies.id, title, img_url, name, array_agg(distinct comments.comments_text) AS comments, ROUND(AVG(distinct stars_rating), 2) AS average_rating FROM movies JOIN genres ON movies.genre_id = genres.id JOIN ratings ON ratings.movieRating_id = movies.id JOIN comments ON comments.movie_id  = movies.id GROUP BY movies.id, title, img_url, name ')
  .then(movies => {
    res.status(200).json({
      movies: movies
    })
...
// JOIN comments ON comments.movie_id = movies.id JOIN ratings ON ratings.movieRating_id = movies.id

const getAllMoviesBasedOnGenre = (req, res, next) => {
  const movieGenreId = req.params.id
  db.any('SELECT movies.id, title, img_url, name FROM movies JOIN genres ON genres.id = movies.genre_id WHERE genres.id=$1 GROUP BY movies.id, title, img_url, name', movieGenreId)
    .then(data => {
      res.status(200).json({
        data: data
      })
